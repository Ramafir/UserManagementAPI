...
export class UserService implements IUserService {
    ...
    async updateMany(usersToUpdate: IUpdateUserDTO[]): Promise<TransactionResult<User[]>> {
        const transaction = await this._userRepository.useTransaction();
        const { manager } = transaction;

        const uniqueRoleIds = getUniqueValuesFromCollection<IUpdateUserDTO, number>(usersToUpdate, 'data.roleId');
        ...
        try {
            ...
            if (roles.length !== uniqueRoleIds.length) {
                throw new TransactionError(`Not all roles are available to perform operation.`);
            }

            const entities = usersToUpdate.map(({ id, data: { isActive, roleId } }) => {
            ...
            return TransactionResult.failure(error instanceof TransactionError ? error.message : undefined);
        } finally {
